...
export default function CreateBookingPage({ setBookingData, next }) {
  ...
  const setSearchUrl = (searchObject) => {
    ...
    searchParams.forEach((value, key) => {
      if (
        !(key in searchObject) ||
        (Array.isArray(searchObject[key]) && searchObject[key].length === 0)
      ) {
  ...
  useEffect(() => {
    Promise.all([getFunction(), getShowFunction(), getFunctions()]).then(
      (data) => {
        setEvents(data[0]);
        setMovies(data[2]);
        if (id) {
  ...
  const initFormData = () => {
    if (payload.current.data.eventId) {
      payload.current.data = {
        ...
        date: moment(payload.current.data.date, "DD MMM YYYY"),
      };
